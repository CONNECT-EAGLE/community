...
| --- | --- | --- |
| `CONNECT-SBG/testrepo` | Public; initial branch creation returned HTTP 403, resolved by installing the connector on the organization. Audited `main` at `c11d82179bb19bac5c0399dfcaaaab3a4d140600`; latest commit 2025-10-29. Sixteen files including drought notebooks, backup/filtered variants, a report, a comparison artifact and requirements. No README, license, citation, workflow or community files in the inspected tree. | Preserve every existing path and blob. Foundation published on `feat/connect-eagle-foundation` through [PR #2](https://github.com/CONNECT-EAGLE/testrepo/pull/2). |
...
Published [PR #2](https://github.com/CONNECT-EAGLE/testrepo/pull/2) from `feat/connect-eagle-foundation` into `main`. The four initial component commits reproduce the prepared local source tree exactly; follow-up changes document publication and address CI findings. Historical paths and blobs remain unchanged. No merge, repository creation, organization rename or repository-settings change is included. Current automation results are available in the [PR checks](https://github.com/CONNECT-EAGLE/testrepo/pull/2/checks).
