...
- GitHub publication succeeded after the connector was installed on the organization: [PR #2](https://github.com/CONNECT-EAGLE/community/pull/2) from `feat/connect-eagle-foundation`, with no direct changes to `main`.
- GitHub Actions runs six jobs: Linux, macOS and Windows on Python 3.11/3.12. The initial run passed all four Linux/macOS jobs and exposed a Windows test-harness limit: pytest used the 64 KiB invalid-YAML fixture as an environment-variable value. Explicit short case IDs correct that failure without reducing fixture size or validation coverage. See the [current PR checks](https://github.com/CONNECT-EAGLE/community/pull/2/checks) for results on the latest commit.
