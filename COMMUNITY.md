# Community and questions

[Home](README.md) · [Start here](START_HERE.md) · [Contribute](CONTRIBUTING.md)

## Ask in the right place

| You need… | Where to go now |
| --- | --- |
| Help with a task | Comment on its issue or PR |
| General onboarding help | [Open a question](https://github.com/CONNECT-SBG/testrepo/issues/15) |
| A bug fixed | [Bug report](https://github.com/CONNECT-SBG/testrepo/issues/new?template=bug-report.yml) |
| Collaborators or mentoring | [Collaboration request](https://github.com/CONNECT-SBG/testrepo/issues/new?template=collaboration-request.yml) |
| Feedback on an idea | [Research idea](https://github.com/CONNECT-SBG/testrepo/issues/new?template=research-idea.yml) |
| A resource shared | [Educational resource proposal](https://github.com/CONNECT-SBG/testrepo/issues/new?template=educational-resource.yml) |

GitHub Discussions is not enabled yet. Until it is, question and collaboration issues provide a public, discoverable fallback. Work remains in actionable issues; when Discussions is available, maintainers can move conversational threads there and leave redirects.

## Meet through useful work

Introduce your interests through a collaboration request linked to a concrete project or question. Students can ask for a first task; researchers can offer a method review; educators can propose a learning resource. No one needs to write code to participate.

The current setup contact is [Fares Alhezaimi (@falhezaimi)](https://github.com/falhezaimi). Use public task threads for routine requests so progress does not depend on direct messages. No other mentors or maintainers are advertised without their agreement.

## Proposed discussion spaces

When activated, keep six categories: **Announcements**, **Help** (Q&A), **Research Ideas**, **Projects & Collaboration**, **Education & Resources**, and **Showcase**. Use VSWIR, TIR, and multi-sensor labels where supported rather than multiplying categories. Announcements summarize actual changes; Showcase links completed work and its contributors.

## Respect and reporting

Follow the [Code of conduct](CODE_OF_CONDUCT.md). The dedicated private conduct-reporting channel and a second moderator still need owner designation. Do not put sensitive reports or personal information in public issues; use GitHub's report-abuse controls for platform abuse. We will publish a community reporting contact after it has been confirmed.
